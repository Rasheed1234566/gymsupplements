...
document.querySelectorAll('.add-to-cart').forEach(button => {
    button.addEventListener('click', (e) => {
        const cartEmoji = document.createElement('div');
        cartEmoji.className = 'cart-animate';
        cartEmoji.textContent = '🛒';

        const rect = e.target.getBoundingClientRect();
        cartEmoji.style.left = rect.left + 'px';
        cartEmoji.style.top = rect.top + 'px';
        cartEmoji.style.position = 'fixed';

        document.body.appendChild(cartEmoji);
